...
ref_po = st.text_input("Ref. PO No.")
date_issue = st.date_input("Date", datetime.now())
...
# ---------------- GENERATE ----------------
if st.button("🚀 Generate & Send Report", use_container_width=True):
    try:
        ...
        if extra:
            ...
            for i in range(0, len(extra), 3):
                ...
                block_height = copy_template_block(ws, ws_temp, start_row)

                img_rows = [5, 18, 31]
                for j, item in enumerate(group):
                    if item["img"]:
                        add_image_to_excel(ws, item["img"], f"A{start_row + img_rows[j] - 1}")
                        write_safe(ws, f"H{start_row + img_rows[j] - 1}", item["desc"])
        ...
        st.success("✅ สร้างรายงาน + ส่งเมลสำเร็จ")
