...
if st.button("🚀 Generate & Send Report", type="primary", use_container_width=True):
    try:
        ...
        # รูปที่ 7+ → clone ImageTemplate ทั้งหน้า
        ...
        for page_idx, page in enumerate(pages, start=1):
            ...
            new_ws.title = f"PhotoPage{page_idx}"

            # ตำแหน่งช่องรูปใน ImageTemplate (ปรับได้ถ้า template คุณต่าง)
            img_cells = ["A5", "A18", "A31"]
            desc_cells = ["H5", "H18", "H31"]

            for i, item in enumerate(page):
                if item["img"]:
                    add_image_to_excel(new_ws, item["img"], img_cells[i])
